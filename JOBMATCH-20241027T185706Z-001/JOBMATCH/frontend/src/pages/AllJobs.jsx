...
function AllJobs({ accountType }) {
    const [jobs, setJobs] = useState([])

    function handleShowJobs() {
        axios.get('http://localhost:8000/show-jobs')
            .then(res => {
        ...
        <div className="flex flex-col w-full h-svh text-blue-950 overflow-hidden">
             <div className="grid grid-cols-12 center p-3 px-6 w-full min-h-24 border-b-4 bg-blue-100 border-blue-400">
                ...
                <div className="col-span-10 flex gap-3 justify-end items-center">
                    <Link to="/all-jobs" className="p-3 rounded-xl bg-blue-400 hover:bg-blue-300">ALL JOBS</Link>
                    {
                        accountType === "Company" &&
            ...
            <div className="flex flex-col p-9 gap-6 w-full h-full overflow-hidden">
                <p className="text-xl font-bold">Find your job!</p>
                <div className="p-1 w-full h-full border-2 rounded-xl border-blue-400 overflow-hidden">
                    <div className="flex flex-col w-full h-full p-6 pr-2 gap-3 border-2 rounded-xl border-blue-400 overflow-y-scroll overflow-hidden scrollable-div">
                        {
                            jobs &&
                            jobs.length > 0 &&
                            jobs.map((job, index) => 
